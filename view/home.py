...
with col1:
    # Search field
    search_by = st.selectbox("Search By", ["", "Company", "Country", "Festivity", "Golf", "Name", "Reception", "BigS/SmallS", "Tier"], key="search_by")
with col2:
    # Search query
...
if search_query:
    if search_by == "":
        ...
        filtered_df = filtered_df[filtered_df['Country'].str.contains(search_query, case=False)]
    elif search_by == "Golf":
        filtered_df = filtered_df[filtered_df['Golf'].str.contains(search_query, case=False)]
    elif search_by == "Reception":
        filtered_df = filtered_df[filtered_df['Reception'].str.contains(search_query, case=False)]
    elif search_by == "Festivity":
        filtered_df = filtered_df[filtered_df['Festivity'].str.contains(search_query, case=False)]
    elif search_by == "BigS/SmallS":
        filtered_df = filtered_df[filtered_df['BigS/SmallS'].str.contains(search_query, case=False)]
    elif search_by == "Tier":
        filtered_df = filtered_df[filtered_df['Tier'].str.lower() == search_query.lower()]
    ...
    with st.container():
        ...
        if pd.notnull(posting_date) and posting_date != 'Nil':
            if isinstance(posting_date, str):
                try:
                    formatted_posting_date = datetime.strptime(posting_date, '%Y-%m-%d %H:%M:%S').strftime('%Y-%m-%d')
                except ValueError:
        ...
        if pd.notnull(deposted_date) and deposted_date != 'Nil':
            if isinstance(deposted_date, str):
                try:
                    formatted_deposted_date = datetime.strptime(deposted_date, '%Y-%m-%d %H:%M:%S').strftime('%Y-%m-%d')
                except ValueError:
        ...
        if not st.session_state[f"edit_mode_{index}"]:
            ...
            st.button(f"Edit {employee_data['Name']}'s info", key=f"edit_{index}", on_click=lambda: toggle_edit_mode(index))
        else:
            # Edit mode
            with st.form(f"edit_form_{index}"):
                # Create form fields for each detail
                new_name = st.text_input("Name", value=employee_data['Name'])
                new_designation = st.text_input("Designation", value=employee_data['Designation'])
                new_status = st.text_input("Status", value=employee_data['Status'])
                new_contact = st.text_input("Contact No.", value=employee_data['Contact No.'])
                ...
                new_posting_date = st.date_input("Posting Date (DD-MM-YY)", value=employee_data['Posting Date'])
                new_depost_date = st.text_input("De-posted Date (YYYY/MM/DD)", value=employee_data['De-posted Date'])
                new_golf = st.text_input("Golf", value=employee_data['Golf'])
                new_golf_handicap = st.text_input("Golf Handicap", value=employee_data['Golf Handicap'])
                new_dietary = st.text_input("Dietary Restrictions", value=employee_data['Dietary Restrictions'])
                new_reception = st.text_input("Reception", value=employee_data['Reception'])
                new_festivity = st.text_input("Festivity", value=employee_data['Festivity'])
                new_interest = st.text_input("Interests", value=employee_data['Interests'])
                new_tier = st.text_input("Tier", value=employee_data['Tier'])
                new_sip = st.text_input("BigS/SmallS", value=employee_data['BigS/SmallS'])
